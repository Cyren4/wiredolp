...
class DNS:
    ...
    def toString(self):
        res = 'DNS:'
        res += '\tIdentification: {}\n\tFlags: val:{}\n\ttext:({})\n\tQuestions: {}\n'.format("0x"+self.identification,"0x"+self.flags,FLAGDICT[self.flags],self.intQuestions)
        res += '\tAnswer RRs: {}\n\tAuthority RRs: {}\n\tAdditional RRs: {}\n'.format(self.intAnswerRRs,self.intAuthorityRRs,self.intAdditionalRRs)
        if(self.intQuestions > 0):
            res += '\tQueries:\n'
            for q in range(self.intQuestions):
                res +='\t\t{}:\n\t\t\tName : {}\n\t\t\tType: {}\n\t\t\tClass: {}\n'.format(q,self.questionNameTab[q],"0x"+self.questionTypeTab[q],"0x"+self.questionClassTab[q])
            
            
            if(self.intAnswerRRs > 0):
                res += '\tAnswers:\n'
                for a in range(self.intAnswerRRs):
                    res+= '\t\t{}:\n\t\t\tName: {}\n\t\t\tType: {}\n\t\t\tClass: {}\n'.format(a,self.answerNameTab[a],"0x"+self.answerTypeTab[a],"0x"+self.answerClassTab[a])
                    res+= '\t\t\tTime to live: {}\n\t\t\tData length: {}\n\t\t\t\n\t\t\tData: {}\n'.format("0x"+self.answerTtlTab[a],"0x"+self.answerDataLengthTab[a],self.answerDataTab[a])
            if(self.intAuthorityRRs > 0):
                res += '\tAuthority:\n'
                for a in range(self.intAuthorityRRs):
                    res+= '\t\t{}:\n\t\tName: {}\n\t\t\tType: {}\n\t\t\tClass: {}\n'.format(a,self.authorityNameTab[a],"0x"+self.authorityTypeTab[a],"0x"+self.authorityClassTab[a])
                    res+= '\t\t\tTime to live: {}\n\t\t\tData length: {}\n\t\t\tData: {}\n'.format("0x"+self.authorityTtlTab[a],"0x"+self.authorityDataLengthTab[a],self.authorityDataTab[a])
            if(self.intAdditionalRRs > 0):
                res += '\tAdditional:\n'
                for a in range(self.intAdditionalRRs):
                    res+= '\t\t{}:\n\t\t\tName: {}\n\t\t\tType: {}\n\t\t\tClass: {}\n'.format(a,self.additionalNameTab[a],"0x"+self.additionalTypeTab[a],"0x"+self.additionalClassTab[a])
                    res+= '\t\t\tTime to live: {}\n\t\t\tData length: {}\n\t\t\tData: {}\n\t\t\t'.format("0x"+self.additionalTtlTab[a],"0x"+self.additionalDataLengthTab[a],self.additionalDataTab[a])
        return res
